...
export default function LanguageDropdown() {
    ...
    const [selectedCode, setSelectedCode] = useState("en"); // initial code, not uppercase string
    ...
    const syncSelectedWithGoogle = () => {
        const currentLang = getCurrentGoogleLang();
        if (currentLang && currentLang !== selectedCode) {
            setSelectedCode(currentLang);
        }
    };
    ...
    const translateTo = (langCode) => {
        ...
        setTimeout(() => {
            if (select) {
                ...
                console.log("translateTo", langCode);
            }
            setLoading(false);
        }, 2000);
    ...
    useEffect(() => {
        ...
        const pollInterval = setInterval(() => {
            ...
            if (select) {
                ...
                syncSelectedWithGoogle();

                select.addEventListener("change", syncSelectedWithGoogle);
        ...
        return () => {
            ...
            const select = document.querySelector("select.goog-te-combo");
            if (select) {
                select.removeEventListener("change", syncSelectedWithGoogle);
            }
        };
    }, [selectedCode]);

    // Find language name from code, fallback to "English"
    const selectedLang = languages.find((l) => l.code === selectedCode)?.name;
    console.log("selectedLang", selectedLang); // Add this lin

    return (
        <div
            ref={dropdownRef}
            className="notranslat md:min-w-[80px] relative"
            translate="no"
        >
            <div
                onClick={() => setOpen(!open)}
                className="hover:bg-[#65365F40] transition-colors duration-300 flex items-center md:ml-3 w-fit md:px-4 py-2 rounded-xl cursor-pointer relative"
            >
                <div className="absolute md:-bottom-3 bottom-2 -right-32 md:-right-3">
                    <span className="text-[10px]">Powered By</span>
                    <svg width="33px" className="-mt-1.5" height="14px" viewBox="0 0 512 168" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMidYMid">
                        <g>
                ...
                {loading ? 'load..' : selectedCode.toUpperCase()}
            </div>

            {open && (
                <div className="absolute md: md:w-24 w-40 space-y-1 top-10 right-0 z-10 bg-[#65365F] rounded-lg overflow-hidden p-2 shadow-md">
                    {languages.map((lang) => (
                        <div
                            key={lang.code}
                            onClick={() => {
                            ...
                            className={`hover:bg-[#27172540] rounded-md transition-colors duration-300 py-1 px-2 cursor-pointer
                ${lang.code === selectedCode ? "bg-[#27172540]" : ""}
                `}
                        >
                            {lang.name}
                        </div>
